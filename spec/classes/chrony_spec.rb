  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'with defaults for all parameters' do
        ...
        it { is_expected.to contain_file('/etc/chrony.conf').with_content(/^port 0$/) }                                                                                                                                                                
        ['0.pool.ntp.org', '1.pool.ntp.org', '2.pool.ntp.org', '3.pool.ntp.org'].each do |s|                                                                                                                                                   
          it { is_expected.to contain_file('/etc/chrony.conf').with_content(/^server #{s} iburst$/) }
        end
        ...
        when 'Debian'
          describe 'install the package' do
            it { is_expected.to contain_package('chrony').with_provider('dpkg') }
          end
        when 'RedHat'
          describe 'install the package' do
            it { is_expected.to contain_package('chrony').with_provider('rpm') }
          end
        when 'Archlinux'
          describe 'install the package' do
            it { is_expected.to contain_package('chrony').with_provider('pacman') }
          end
        end
        ...
        context 'with some params passed in' do
          ...
          it { is_expected.to contain_file('/etc/chrony.conf').with_content(/^port 123$/) }
					it { is_expected.to contain_file('/etc/chrony.conf').with_content(/^allow 192\.168\/16$/) }
        ...
        context 'unmanaged chrony.keys file' do
          ...
          it { expect {
            should compile
          }.to raise_error(/The chrony module is not supported on an UnsupportedOS based system\./) }
				end
			end
    end
	end
